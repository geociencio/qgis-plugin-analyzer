...
*   **`docs/CHANGELOG.md`**: Record user-visible changes in `[Unreleased]` using the **changelog-generator** skill based on session commits. **(NOTE: The `changelog-generator` skill is interpretive [prompt-based]. DO NOT attempt to run any Python scripts. Instead, use `git log` to read the recent session commits, analyze them, and use the file editing tool to write directly to CHANGELOG.md)**.
