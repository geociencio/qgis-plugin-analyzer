...
def _build_markdown_header(analyses, module_score, maint_score, project_type):
    """Builds the markdown header section."""
...
def _build_markdown_qgis_findings(analyses):
    """Builds the QGIS findings section."""
    from typing import List
...
def _build_markdown_semantic_section(semantic):
    """Builds the semantic analysis section."""
    from typing import List
...
def _build_markdown_repo_standards(analyses):
    """Builds the repository standards section."""
    from typing import List
...
def generate_markdown_summary(analyses: Dict[str, Any], output_path: pathlib.Path):
    ...
    module_score = metrics.get("quality_score", 0)
    maint_score = metrics.get("maintainability_score", 0)

    # Build header
    lines = _build_markdown_header(analyses, module_score, maint_score, project_type)

    # QGIS-specific findings
    if project_type == "qgis":
        lines.extend(_build_markdown_qgis_findings(analyses))

    # Semantic analysis
    semantic = analyses.get("semantic", {})
    lines.extend(_build_markdown_semantic_section(semantic))

    # Repository standards (QGIS only)
    if project_type == "qgis":
        lines.extend(_build_markdown_repo_standards(analyses))
...
def _build_html_header(
    ...
    """Builds the HTML header section."""
    html = [
        "<!DOCTYPE html>",
...
def _build_html_qgis_findings(analyses):
    """Builds the QGIS findings HTML section."""
...
def _build_html_semantic_section(semantic):
    """Builds the semantic analysis HTML section."""
...
def _build_html_repo_compliance(repo_comp):
...
def _build_html_ruff_findings(ruff_findings):
...
def _build_html_general_metrics(metrics):
...
def generate_html_report(analyses: Dict[str, Any], output_path: pathlib.Path):
    ...
    # QGIS Findings
    if project_type == "qgis":
        html.extend(_build_html_qgis_findings(analyses))

    # Semantic Section
    semantic = analyses.get("semantic", {})
    if semantic:
        html.extend(_build_html_semantic_section(semantic))

    # Repository Compliance
    repo_comp = analyses.get("repository_compliance", {})
    if repo_comp:
        html.extend(_build_html_repo_compliance(repo_comp))

    # Ruff Findings
    if ruff_findings:
        html.extend(_build_html_ruff_findings(ruff_findings))

    # General Metrics
    html.extend(_build_html_general_metrics(metrics))
